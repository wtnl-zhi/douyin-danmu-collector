...
async function exportWordCloud3dBillboard(sessionId: string, minFrequency: number) {
  const data = wordCloud(sessionId, minFrequency); const pick = await dialog.showSaveDialog({ defaultPath: `弹幕关键词云-${Date.now()}.html` }); if (pick.canceled || !pick.filePath) return null
  const words = JSON.stringify(data.words).replaceAll('<', '\\u003c'); const list = data.words.slice(0, 50).map((item, index) => `<tr><td>${String(index + 1).padStart(2, '0')}　${item.word}</td><td>${item.count}</td></tr>`).join('')
  await writeFile(pick.filePath, `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>3D 弹幕词云</title><style>*{box-sizing:border-box}body{margin:0;background:#07111f;color:#e9f5ff;font-family:-apple-system,"Noto Sans SC",sans-serif}.wrap{max-width:1160px;margin:auto;padding:42px 28px}.eyebrow{font:12px ui-monospace;color:#65b6ff;letter-spacing:2px}h1{margin:8px 0;font-size:30px}.sub{color:#9abbd7;font-size:13px}.panel{display:grid;grid-template-columns:1fr 210px;overflow:hidden;border:1px solid #2b4d6d;border-radius:22px;background:#091a2d;box-shadow:0 24px 65px #0006}.stage{position:relative;min-height:520px;overflow:hidden;perspective:900px;background:radial-gradient(ellipse at center,#164d81,#0b2037 45%,#071525)}.stage:before{content:"";position:absolute;inset:0;background-image:linear-gradient(#b3ddff0c 1px,transparent 1px),linear-gradient(90deg,#b3ddff0c 1px,transparent 1px);background-size:26px 26px}.sphere{position:absolute;left:50%;top:48%;width:0;height:0;transform-style:preserve-3d;animation:orbit 32s cubic-bezier(.45,.02,.55,.98) infinite}.core{position:absolute;width:340px;height:340px;border-radius:50%;transform:translate3d(-170px,-170px,-18px);background:radial-gradient(circle at 32% 25%,#c3ecff25,#65baff14 24%,transparent 70%);box-shadow:inset -22px -16px 50px #0005,0 0 75px #3da7ff24}.word{position:absolute;left:0;top:0;white-space:nowrap;font-weight:700;color:#65b7ff;transform-style:preserve-3d}.word:nth-child(3n){color:#9bd5ff}.word:nth-child(4n){color:#3e9dff}.word.featured{color:#eff9ff;font-weight:800}.face{display:inline-block;text-shadow:0 1px 0 #061422,0 0 18px #32a0ff52;backface-visibility:hidden;transform-style:preserve-3d;animation:counter 32s cubic-bezier(.45,.02,.55,.98) infinite}.stage:hover .sphere,.stage:hover .face{animation-play-state:paused}.hint{position:absolute;bottom:17px;left:18px;color:#80aaca;font-size:10px}.rank{padding:22px;background:#091a2dbd;border-left:1px solid #2b4d6d}.rank h2{font:11px ui-monospace;color:#76b5e8;letter-spacing:1.6px}.rank table{width:100%;border-collapse:collapse;font-size:12px}.rank td{padding:8px 0;border-bottom:1px solid #23415e}.rank td:last-child{text-align:right;color:#62b4ff}@keyframes orbit{0%{transform:rotateX(-9deg) rotateY(0deg) rotateZ(-1deg)}42%{transform:rotateX(12deg) rotateY(152deg) rotateZ(1deg)}67%{transform:rotateX(-4deg) rotateY(238deg) rotateZ(-2deg)}100%{transform:rotateX(-9deg) rotateY(360deg) rotateZ(-1deg)}}@keyframes counter{0%{transform:translate(-50%,-50%) rotateZ(1deg) rotateY(0deg) rotateX(9deg)}42%{transform:translate(-50%,-50%) rotateZ(-1deg) rotateY(-152deg) rotateX(-12deg)}67%{transform:translate(-50%,-50%) rotateZ(2deg) rotateY(-238deg) rotateX(4deg)}100%{transform:translate(-50%,-50%) rotateZ(1deg) rotateY(-360deg) rotateX(9deg)}}@media(max-width:700px){.panel{grid-template-columns:1fr}.rank{border-left:0;border-top:1px solid #2b4d6d}}</style><main class="wrap"><p class="eyebrow">LIVE LANGUAGE ORB</p><h1>3D 弹幕词云</h1><p class="sub">词面始终朝向镜头 · 缓慢自转 · 悬停暂停</p><section class="panel"><div class="stage"><div class="sphere" id="sphere"><i class="core"></i></div><div class="hint">缓慢自转 · 悬停暂停</div></div><aside class="rank"><h2>TOP 50</h2><table>${list}</table></aside></section></main><script>const data=${words},s=document.getElementById('sphere'),r=164,f=[[0,-4,164],[-87,-68,130],[91,-62,128],[-112,25,104],[111,30,104],[-62,100,120],[57,98,120],[0,-122,95]];data.slice(0,46).forEach((item,i)=>{let x,y,z,op=1;const size=Math.max(14,Math.min(70,item.size+(i<3?10:i<8?3:0)));if(i<f.length)[x,y,z]=f[i];else{const o=i-f.length,t=Math.max(data.length-f.length,1),p=Math.acos(1-2*((o+.45)/t)),a=Math.PI*(1+Math.sqrt(5))*o;x=r*Math.sin(p)*Math.cos(a);y=r*Math.cos(p);z=r*Math.sin(p)*Math.sin(a);op=.46+((z+r)/(r*2))*.54}const e=document.createElement('span');e.className='word'+(i<3?' featured':'');e.style.fontSize=size+'px';e.style.opacity=op;e.style.transform='translate3d('+x+'px,'+y+'px,'+z+'px)';e.innerHTML='<span class="face">'+item.word+'</span>';s.appendChild(e)})</script></html>`)
  return pick.filePath
}
